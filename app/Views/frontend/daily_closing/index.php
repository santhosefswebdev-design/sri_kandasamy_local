								...
								<div class="header">
									<form action="<?php echo base_url(); ?>/dailyclosing_online" method="post">
										<div class="row" style="margin: 10px 0px">
											...
											<?php if(!empty($cur_float_cash_cnt)){ ?>
												<div class="col-md-2" align="right">
													<button type="button" class="btn btn-info btn-lg ar_btn" data-toggle="modal" data-target="#closingCashModal">Closing Cash</button>
												</div>
											<?php }else{ ?>
												<div class="col-md-2" align="right">
													<button type="button" class="btn btn-info btn-lg ar_btn" data-toggle="modal" data-target="#openingCashModal">Opening Cash</button>
												</div>
											<?php } ?>
									...
									<div class="modal fade" id="openingCashModal" tabindex="-1" role="dialog" aria-labelledby="openingCashModalLabel" aria-hidden="true">
                                        <div class="modal-dialog" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="floatingCashModalLabel">Floating Cash Entry</h5>
                                                ...
                                                <div class="modal-body">
                                                    <form id="floatingOpenForm">
														<div class="form-group"><label>Previous day Closing Balance: <?php echo (isset($prev_float_cash->closing) ? (float) $prev_float_cash->closing : 0); ?></label></div>
                                                        <div class="form-group">
                                                            <label for="opening">Opening Float Cash:</label>
                                                            <input type="number" class="form-control" id="opening" name="opening" placeholder="Enter Float Cash" required>
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="checked_op">Checked By:</label>
                                                            <input type="text" class="form-control" id="checked_op" name="checked_op" placeholder="Enter Name" required>
                                                        </div>
                                                    </form>
                                                </div>
                                                ...
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                                                    <button type="submit" class="btn btn-primary" onclick="saveOpenFloat()">Submit</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
												...
												<tbody>
													...
													foreach ($hallbooking_details as $hallbooking_detail) {
														...
														<tr>
															...
															<td style="padding: 5px 10px!important;">
																<?php echo date('d/m/Y', strtotime($hallbooking_detail['date'])); ?>
															</td>
															<td style="padding: 5px 10px!important;">
																<?php
												...
												<tbody>
													...
													foreach ($ubayam_details as $ubayam_detail) {
														...
														<tr>
															...
															<td style="padding: 5px 10px!important;">
																<?php echo date('d/m/Y', strtotime($ubayam_detail['date'])); ?>
															</td>
															<td style="padding: 5px 10px!important;">
																<?php
...
<?php
												...
												<tbody>
													...
													foreach ($repayment_details as $details) {
														...
														<tr>
															...
															<td style="padding: 5px 10px!important;">
																<?php echo date('d/m/Y', strtotime($details['date'])); ?>
															</td>
							...
							<div class="modal fade" id="closingCashModal" tabindex="-1" role="dialog" aria-labelledby="closingCashModalLabel" aria-hidden="true">
								<div class="modal-dialog" role="document">
									<div class="modal-content">
										<div class="modal-header">
											<h5 class="modal-title" id="floatingCashModalLabel">Floating Cash Entry</h5>
										...
										<div class="modal-body">
											<form id="floatingCloseForm">
												<div class="form-group"><label>Opening Balance (<?php echo date('d-m-Y') ?>): <?php echo (isset($cur_float_cash_details->opening) ? number_format((float) $cur_float_cash_details->opening, 2) : 0); ?></label></div>
												<div class="form-group"><label>Cash Income: <?php echo number_format($summary_totals_income['CASH'], 2); ?><input type="hidden" name="income" value="<?php echo $summary_totals_income['CASH']; ?>" /></label></div>
												<div class="form-group"><label>Cash Expense: <?php echo number_format($summary_totals_expense['CASH'], 2); ?><input type="hidden" name="expense" value="<?php echo $summary_totals_expense['CASH']; ?>" /></label></div>
												<div class="form-group"><label>Expected Closing Balance: <?php echo number_format($cash_drawer, 2); ?></label></div>
												<div class="form-group">
													<label for="closing">Closing Float Cash:</label>
													<input type="number" class="form-control" id="closing" name="closing" value="<?php echo isset($cur_float_cash_details->closing) ? number_format($cur_float_cash_details->closing, 2) : ''; ?>" placeholder="Enter Float Cash" required>
												</div>
												<div class="form-group">
													<label for="checked_cl">Checked By:</label>
													<input type="text" class="form-control" id="checked_cl" value="<?php echo !empty($cur_float_cash_details->checked_cl) ? $cur_float_cash_details->checked_cl : ''; ?>" name="checked_cl" placeholder="Enter Name" required>
												</div>
											</form>
										</div>
										...
										<div class="modal-footer">
											<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
											<button type="submit" class="btn btn-primary" onclick="saveCloseFloat()">Submit</button>
										</div>
									</div>
								</div>
							</div>
...
<script>
    function saveOpenFloat() {
        const openingFloat = document.getElementById('opening').value;
        const checked_op = document.getElementById('checked_op').value;

        if (openingFloat && checked_op) {
            // Send data via AJAX
            $.ajax({
                url: "<?php echo base_url(); ?>/Dailyclosing_online/saveOpenFloat", // Your endpoint
				type: "POST",
				data: {
	...
	function saveCloseFloat() {
		var form_data = $('#floatingCloseForm').serialize()

		// Send data via AJAX
		$.ajax({
			url: "<?php echo base_url(); ?>/Dailyclosing_online/saveCloseFloat", // Your endpoint
			type: "POST",
			data: form_data,
			success: function (response) {
				if (response.status === 'success') {
					alert(response.message);
					$('#closingCashModal').modal('hide');
					location.reload();
				} else {
					alert(response.message);
				}
