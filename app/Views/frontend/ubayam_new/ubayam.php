...
<body class="sidebar-icon-only">
 ...
 <div class="row prod product" id="portfoliolist">
            ...
            foreach ($value as $row) {
                ...
                <div class="portfolio <?php if (!empty($key)) {
                    ...
                    <label class="card" for="pay_for<?php echo $row['id']; ?>" onclick="payfor(<?php echo $row['id']; ?>)">
                        <img class="img-fluid prod_img" src="<?php echo base_url(); ?>/uploads/ubayam/<?php echo $row['image']; ?>">
    ...
    <div class="col-md-6 paid_ro" style="padding: 0px;">
        ...
        <div class="col-md-12">
            <input type="number" style="font-size:14px;height: 35px;border: 1px solid #cccccc;text-align: center;font-size: 24px;font-weight: bold;" 
            name="paid_amount" id="paid_amount" placeholder="0.00" class="form-control paid_amount" onkeyup="calculateBalance()">
        </div>
    </div>
    <div class="col-md-6 paid_ro" style="padding: 0px;">
        ...
        <div class="col-md-12">
            <input type="number" style="font-size:14px;height: 35px;border: 1px solid #cccccc;text-align: center;font-size: 24px;font-weight: bold;" 
            name="balance_amount" id="balance_amount" placeholder="0.00" class="form-control" readonly>
        </div>
    </div>
</div>
...
.payment1 li label { padding: 15px 1px; margin: 15px 5px; }
  ...
  .cal_head {
   ...
   function calculateBalance() {
        // Get the total amount and the paid amount
        var total_amt = parseFloat(document.getElementById("total_amt").value);
        var paid_amt = parseFloat(document.getElementById("paid_amount").value);

        // Check if both values are numbers
        if (!isNaN(total_amt) && !isNaN(paid_amt)) {
            ...
            // Reset balance if inputs are invalid
            document.getElementById("balance_amount").value = '0.00';
        }
    }

    // Ensure partial payment validation when form is submitted
            ...
            function displayCart() {
                ...
                if (cartArray.length > 0) {
					...
					output += '<input type="hidden" name="name" value="' + cartArray[0].name + '" />\
						<input type="hidden" name="email_id" value="' + cartArray[0].email_id + '" />\
