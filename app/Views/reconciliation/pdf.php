...
<h2 style="text-align:center;">RECONCILIATION REPORT AT <?php echo date('M, Y', strtotime($pdfdata['recon_month'] . '-01')); ?></h2>
<table style="width:100%;">
	<thead>
		<tr>
			<td>ID#</td>
	...
	if(count($pdfdata['pending_debit_transactions']) > 0){
		echo '<tr><td colspan="5">&nbsp;</td></tr>';
		echo '<tr><td colspan="5">Outstanding Charges</td></tr>';
		echo '<tr><td colspan="5">&nbsp;</td></tr>';
		foreach($pdfdata['pending_debit_transactions'] as $pbt){
	...
	if(count($pdfdata['pending_credit_transactions']) > 0){
		echo '<tr><td colspan="5">&nbsp;</td></tr>';
		echo '<tr><td colspan="5">Outstanding Payments</td></tr>';
		echo '<tr><td colspan="5">&nbsp;</td></tr>';
		foreach($pdfdata['pending_credit_transactions'] as $pct){
	...
	<table>
		<tbody>
			<tr>
				...
				<td align="right"><?php if($pdfdata['closing_balance'] > 0) $pdfdata['closing_balance_amount'] = number_format(abs($pdfdata['closing_balance']), 2); else $pdfdata['closing_balance_amount'] = '(' . number_format(abs($pdfdata['closing_balance']), 2) . ')'; echo $pdfdata['closing_balance_amount']; ?></td>
			</tr>
			<tr>
			...
			<tr>
				<td>Subtotal:</td>
				<td align="right"><?php $subtotal = $pdfdata['closing_balance'] - $outstanding_charges; if($subtotal > 0) $subtotal_amount = number_format(abs($subtotal), 2); else $subtotal_amount = '(' . number_format(abs($subtotal), 2) . ')'; echo $subtotal_amount ; ?></td>
			</tr>
			<tr>
