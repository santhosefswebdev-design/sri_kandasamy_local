...
<section class="content">
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" name="receipt_receivedfrom" id="receipt_receivedfrom" value="<?php if (!empty($entries['paid_to'])) {
...
$(document).ready(function(){
    ...
    $('#adddebitdetail').click(function(e){
        e.preventDefault(); // Prevent default action
        
        var receipt_debit_ac = $("#receipt_debit_ac").val();
        var receipt_amount = $("#receipt_amount").val();
        
        console.log('Debit AC:', receipt_debit_ac); // Debug log
        console.log('Amount:', receipt_amount); // Debug log
        
        // Validate inputs
        if(!receipt_debit_ac || receipt_debit_ac == '' || receipt_debit_ac == '0'){
            alert('Please select a Credit A/C');
            return false;
        }
        
        if(!receipt_amount || receipt_amount == "0.00" || receipt_amount == "0" || parseFloat(receipt_amount) <= 0){
        ...
        $('.all_close').each(function(){
            if($(this).attr('data-id') == receipt_debit_ac){
                isDuplicate = true;
                return false;
            }
        });
        ...
        var amt = Number(receipt_amount).toFixed(2);
        
        var text1 = '<tr class="all_close" data-id="'+debit_ac+'" id="remov'+i+'">';
        ...
        text1 += '<td><input type="hidden" style="text-align: center;" id="ledgerid_'+i+'" class="row_amt" name="entries['+i+'][ledgerid]" value="'+debit_ac+'">';
        text1 += '<select class="form-control search_box" data-live-search="true" data-live-search-style="startsWith" id="receipt_debit_ac_new_'+i+'" onchange="reselectledger('+i+',this.value)">';
        text1 += '<option value="">Select credit a/c</option>';
        <?php if (!empty($ledgers)) {
			...
			$("#receipt_debit_ac_new_" + i).val(debit_ac);
