...
<section class="content">
    <div class="container-fluid">
										...
										<div class="form-group">
											...
											<select class="form-control" name="payment_paymode" id="payment_paymode" <?php echo $disabled; ?>>
												...
												<?php foreach ($payment_mode as $key => $pay) { ?>
													<option value="<?php echo $pay['id']; ?>" <?php if(!empty($entries['payment'])){ if($entries['payment'] == $pay['name']){ echo "selected"; }} ?>> <?php echo $pay['name']; ?> </option>
												<?php } ?>

												<?php /*<option value="cash" <?php if(!empty($entries['payment'])){ if($entries['payment'] == "cash"){ echo "selected"; }} ?>>Cash</option>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="payment_receivedfrom" id="payment_receivedfrom" value="<?php if (!empty($entries['paid_to'])) {
													echo $entries['paid_to'];
...
document.getElementById('payment_date').addEventListener('change', function () {
    ...
    if (selectedDate) {
        $.ajax({
            url: '<?php echo base_url(); ?>/entries/generatePaymentEntryCodeByDate',
				type: 'POST',
	...
	// FIXED ADD CREDIT DETAIL FUNCTION
	$(document).ready(function () {
		var i = 1;

		$('#addcreditdetail').click(function (e) {
			e.preventDefault(); // Prevent default action

			var payment_credit_ac = $("#payment_credit_ac").val();
			var payment_amount = $("#payment_amount").val();

			console.log('Credit AC:', payment_credit_ac); // Debug log
			console.log('Amount:', payment_amount); // Debug log

			// Validate inputs
			if (!payment_credit_ac || payment_credit_ac == '' || payment_credit_ac == '0') {
				alert('Please select a Debit A/C');
				return false;
			}

			if (!payment_amount || payment_amount == "0.00" || payment_amount == "0" || parseFloat(payment_amount) <= 0) {
			...
			$('.all_close').each(function () {
				if ($(this).attr('data-id') == payment_credit_ac) {
					isDuplicate = true;
					return false;
				}
			});
			...
			if (isDuplicate) {
				alert('This ledger is already added');
				return false;
			}

			i++;
			var credit_ac = payment_credit_ac;
			var amt = Number(payment_amount).toFixed(2);

			var text1 = '<tr class="all_close" data-id="' + credit_ac + '" id="remov' + i + '">';
			text1 += '<td><input type="hidden" name="entries[' + i + '][entryitemid]">';
			text1 += '<a class="btn btn-info" style="font-size: 15px;cursor: pointer;color: #fff;font-weight: bold;padding: 0px 5px;" onclick="remove(' + i + ')" id="remove">X</a></td>';
			text1 += '<td><input type="hidden" style="text-align: center;" id="ledgerid_' + i + '" class="row_amt" name="entries[' + i + '][ledgerid]" value="' + credit_ac + '">';
			text1 += '<select class="form-control search_box" data-live-search="true" data-live-search-style="startsWith" id="payment_credit_ac_new_' + i + '" onchange="reselectledger(' + i + ',this.value)">';
			text1 += '<option value="">Select debit a/c</option>';
			<?php if (!empty($ledgers)) {
				foreach ($ledgers as $ledger) { ?>
					text1 += '<option value="<?php echo $ledger["id"]; ?>"><?php echo addslashes($ledger['left_code'] . '/' . $ledger['right_code'] . " - " . $ledger["name"]); ?></option>';
				<?php }
			} ?>
			text1 += '</select></td>';
			...
			$(".cart-table tbody").append(text1);

			// Set selected value and refresh
			$("#payment_credit_ac_new_" + i).val(credit_ac);
