...
$("#submit").click(function(){
	...
	// FIXED ADD CREDIT DEBIT DETAIL FUNCTION
	$(document).ready(function () {
		var i = 1;

		$('#addCreditDebitdetail').click(function (e) {
			...
			console.log('Account Name:', account_name); // Debug log
			console.log('Debit Amount:', dramt); // Debug log
			console.log('Credit Amount:', cramt); // Debug log

			// Validate inputs
			if (!account_name || account_name == '' || account_name == '0') {
				alert('Please select a Ledger Name');
				return false;
			}

			if ((dramt == "0.00" || dramt == "0") && (cramt == "0.00" || cramt == "0")) {
			...
			$('.all_close').each(function () {
				if ($(this).attr('data-id') == account_name) {
					isDuplicate = true;
					return false;
				}
			});
			...
			// Determine readonly states
			var cramt_read = '';
			var dramt_read = '';

			if (parseInt(dramt) == "0" || dramt == "" || dramt == "NaN") {
				cramt_read = ' style="text-align: center;"';
			} else {
				cramt_read = ' style="text-align: center; background-color: #f6f6f6;" readonly';
			}

			if (parseInt(cramt) == "0" || cramt == "" || cramt == "NaN") {
				dramt_read = ' style="text-align: center;"';
			} else {
				dramt_read = ' style="text-align: center; background-color: #f6f6f6;" readonly';
			}

			var text1 = '<tr class="all_close" data-id="' + account_name + '" id="remov' + i + '">';
			text1 += '<td style="width:10%;"><a class="btn btn-info" style="font-size: 15px;cursor: pointer;color: #fff;font-weight: bold;padding: 0px 5px;" onclick="remove(' + i + ')" id="remove">X</a></td>';
			text1 += '<td style="width: 30%;"><input type="hidden" style="text-align: center;" id="ledgerid_' + i + '" class="row_amt" name="entries[' + i + '][ledgerid]" value="' + account_name + '">';
			text1 += '<select class="form-control search_box" data-live-search="true" data-live-search-style="startsWith" id="journal_accountname_new_' + i + '" onchange="reselectledger(' + i + ',this.value)">';
			text1 += '<option value="">Select ledger</option>';
			...
			$("#journal_accountname_new_" + i).val(account_name);
