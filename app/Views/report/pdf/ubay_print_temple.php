...
<table align="center" style="width: 100%;max-width: 800px;">
				...
				<tr>
					...
					<td width="85%" align="left">
						...
						<p style="text-align:left; font-size:16px; margin:5px 0px;">
							<?php echo $_SESSION['address1']; ?>, <br><?php echo $_SESSION['address2']; ?>,<br>
	...
	// IMPORTANT: Extract event date filters from pdfdata
	$event_date_from = isset($pdfdata['event_date_from']) && !empty($pdfdata['event_date_from']) ? $pdfdata['event_date_from'] : null;
	$event_date_to = isset($pdfdata['event_date_to']) && !empty($pdfdata['event_date_to']) ? $pdfdata['event_date_to'] : null;
	?>
	<tr>
		<td colspan="2" style="width: 100%;">
			<h3 style="text-align:center;">UBAYAM VOUCHER
...
<table border="1" width="100%" align="center">
	...
	<tbody>
		...
		// Convert event dates to Y-m-d format for database query
		$event_from_db = !empty($event_date_from) ? date('Y-m-d', strtotime($event_date_from)) : null;
		$event_to_db = !empty($event_date_to) ? date('Y-m-d', strtotime($event_date_to)) : null;
		...
		// APPLY EVENT DATE FILTERS - THIS IS THE KEY ADDITION!
		if (!empty($event_from_db)) {
			$dat = $dat->where('templebooking.booking_date >=', $event_from_db);
		}
		if (!empty($event_to_db)) {
			$dat = $dat->where('templebooking.booking_date <=', $event_to_db);
		}
