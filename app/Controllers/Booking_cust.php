		...
		if( !empty($data['booking_date']) && !empty($data['event_name']) && !empty($data['name']) && !empty($data['mobile_number'])) {
			...
			if($res){
				...
				if(!empty($_POST['email'])){
						$temple_title = "Temple ".$_SESSION['site_title_frend'];
					  $qr_url = base_url()."/booking_cust/reg/";
					  ...
					  echo $message;
					  $subject = $_SESSION['site_title_frend']." HALL BOOKING";
					  $to_user = $_POST['email'];
